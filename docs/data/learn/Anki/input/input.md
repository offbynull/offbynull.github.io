 ...
 * `{ankiAnswer} retrovirus` - A virus that inserts a DNA copy of its RNA genome into the DNA of the host cell that it invades.

 * `{ankiAnswer} (anti[\-]?viral|anti[\-]?retroviral)/i` - A class of medication used for treating viral infections.
 ...
 * `{ankiAnswer} surface protein` - Protein embedded into a cell surface or viral envelope. See glycoprotein / glycan.

 * `{ankiAnswer} (viral envelope|viral envelope protein|envelope protein)/i` - One of the proteins making up the outermost layer of a virus, called the viral envelope.
 ...
 * `{ankiAnswer} glycoprotein` - A protein containing glycans.

 * `{ankiAnswer} glycan` - A carbohydrate portion of some glycoconjugate (e.g. glycoprotein or glycolipid). Cells have a dense coating of glycans on their surface, which are used for modulating interactions with other cells and biological entities (e.g. communication between the cells of a human, interactions between bacterial cells and human cells, interactions between a human cells and viruses, etc..). 

   Glycans may also coat viral envelope proteins, which can make those viruses invisible to the human immune system (e.g. HIV).

 * `{ankiAnswer} glycosylation` - A modification to a protein, applied after it's already been translated out of the ribosome, that turns it into a glycoprotein.

 * `{ankiAnswer} (Red Queen effect|Red Queen hypothesis)/i` - The hypothesis that organisms must constantly evolve in order to survive due to predator-prey dynamics within an environment. The name comes from Lewis Carroll's novel Through the Look-Glass, where the Red Queen tells Alice "Now, here, you see, it takes all the running you can do, to keep in the same place."

 * `{ankiAnswer} (syncytium|symplasm)/i` - A cytoplasmic mass containing several nuclei formed by the fusion of multiple cells. Certain HIV phenotypes embed their viral envelope proteins into the host cell's surface upon infection, which ends up causing neighbouring cells to fuse into a non-functional syncytium.

 * `{ankiAnswer} (odds[\-\s]ratio)/i` - A measure of the chance of success, defined as the probability of some event occurring divided by the probability that event doesn't occur. For example, given a based coin, a fair coin, and a sequence of flips, the probability the sequence of flips was generated by the ...
 ...
 * `{ankiAnswer} (log[-\s]?odds[-\s]?ratio)/i` - The logarithm of the odds ratio: `{kt} log_2(\frac{Pr_B(flips)}{Pr_F(flips)})`. Log-odds ratio is just another representation of odds ratio, typically used in cases when odds ratio generates a very small / large result.
 ...
 * `{ankiAnswer} methylation` - The addition of a methyl group (CH3) to a cytosine or guanine nucleotide.
 ...
 * `{ankiAnswer} (C[p\-]?G island|C[p\-]?G site)/i` - Regions of DNA with a high frequency of cytosine followed by guanine. The reverse complementing strand will have equal regions with equally high frequencies of guanine followed by cytosine.

 * `{ankiAnswer} Hidden Markov Model` - A model of a machine that outputs a sequence.
 ...
 * `{ankiAnswer} Hidden Markov Model diagram` - A visualization of an HMM as a directed graph.
 ...
 * `{ankiAnswer} hidden state` - A state within an HMM. At any given time, a HMM will be in one of n different hidden states. Unless a hidden state is a non-emitting hidden state, ...
 ...
 * `{ankiAnswer} emitting hidden state` - A hidden state that emits a symbol. An HMM typically emits a symbol after transitioning between hidden states. However, if the hidden state being transitioned to is a non-emitting hidden state, it doesn't emit a symbol.

 * `{ankiAnswer} (non-emitting hidden state|silent hidden state)/i` - A hidden state that doesn't emit symbols. An HMM typically emits a symbol after transitioning between hidden states. However, if the hidden state being transitioned to is a non-emitting hidden state, it doesn't emit a symbol.
 ...
 * `{ankiAnswer} hidden path` - A sequence of hidden state transitions that a HMM passes through. For example, in the HMM diagram below, one possible hidden path could be as follows:
 ...
 * `{ankiAnswer} (symbol emission|emitted symbol)/i` - A symbol emitted after a hidden state transition. The HMM diagram below can emit the symbols [hit, miss, foul].
 ...
 * `{ankiAnswer} (symbol emission sequence|emission sequence|emitted sequence)/i` - A sequence of symbol emissions, where those symbols are emitted from an HMM. The HMM diagram below can produce the emitted sequence ...
 ...
 * `{ankiAnswer} (Viterbi algorithm|Viterbi graph|Viterbi)/i` - An algorithm that determines the most probable hidden path in an HMM for some emitted sequence. 
 ...
 * `{ankiAnswer} exploded HMM` - A transformation of an HMM such that hidden state transitions are enumerated based on an emitted sequence (HMM cycles removed).
 ...
 * `{ankiAnswer} Viterbi learning` - A Monte Carlo algorithm that uses the Viterbi algorithm to derive an HMM's probabilities from an emitted sequence.
