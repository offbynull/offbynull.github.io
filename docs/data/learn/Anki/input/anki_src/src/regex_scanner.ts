...
export class RegexScanner {
    ...
    static create(regex: string, flags: string) {
        ...
        if (group1Exists === false || extraGroupsExist === true) {
            throw new Error('Bookmark regex must contain exactly 1 explicit capture group (not more or less): ' + regex);
        }
    ...
    static createFromLiteral(regexLiteral: string) {
        ...
        return RegexScanner.create(parsedRegexLiteral.regex, parsedRegexLiteral.flags);
    }
    ...
    public scan(text: string): RegexCapture | null {
        ...
        for (let i = 1; i < this.groupMappings.length; i++) {
            ...
            if (groupMapping === 1) {
                if (groupText.length === 0) {
                    throw new Error('Bookmark regex must capture at least 1 character (it captured nothing): ' + this.originalRegex);
                }
        ...
        throw new Error('This should never happen'); // we have have at least capture group 1
    }
}