...
with open('PAM250.txt', mode='r', encoding='utf-8') as f:
    data = f.read()
...
# Load sequences
#
with open('/home/user/Downloads/dataset_240305_10.txt', mode='r', encoding='utf-8') as f:
    data = f.read()
...
from_node = grid[0][0]
to_node = grid[-1][-1]
for row in grid:  # create free-rides from source node to every other node
    for dst_n in row:
        if dst_n == from_node:
            continue
        g.insert_edge(next_edge_id, from_node, dst_n, [ModificationType.SKIP, 0])
        next_edge_id += 1
for row in grid:  # create free-rides from every node to dst node
    for src_n in row:
        if src_n == to_node:
            continue
