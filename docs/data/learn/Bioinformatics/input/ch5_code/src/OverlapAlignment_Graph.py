...
def overlap_alignment(v: List[ELEM], w: List[ELEM], weight_lookup: WeightLookup) -> Tuple[float, List[List[ELEM]]]:
    ...
    for w_idx in range(0, w_node_count - 1):
        graph.insert_edge(
            sink_free_ride_id_create_func(),
            (v_node_count - 1, w_idx),
            to_node,
            FreeRideEdgeData()
        )
...
if __name__ == '__main__':
    s1 = list('CATCATCATGATGGCA')
    s2 = list('AGTAGGCTTAAGGTTA')
    alignment = overlap_alignment(s1, s2, Constant2DWeightLookup(1, -1, -1))
