...
for layer in grid.values():  # create free-rides from source node to every other node
    for row in layer:
        for dst_n in row:
            if dst_n == from_node:
                continue
            g.insert_edge(
                ...
                from_node,
                dst_n,
                [ModificationType.SKIP, 0]
            )
for layer in grid.values():  # create free-rides from every node to dst node
    for row in layer:
        for src_n in row:
            if src_n == to_node:
                continue
