...
# MARKDOWN
def probability_of_state_transitions(
        hmm: Graph[N, ND, E, ED],
        source_state: STATE,
        states: list[STATE],
    ...
    to_state = states[0]
    weight = get_edge_transition_prob(hmm, source_state, to_state)
...
def main():
    ...
    try:
        ...
        p = probability_of_state_transitions(
            hmm,
            source_state,
            states,
        ...
        print('```')
        print()
        print(f'Starting from {source_state} and going through {states} resulted in probability {p}')
        print()
    finally:
        print("</div>", end="\n\n")
