...
with open('/home/user/Downloads/dataset_240343_8(4).txt', mode='r', encoding='utf-8') as f:
    data = f.read()
...
g_per_idx = []
for i in range(seq_len):
    g: UndirectedGraph.Graph[str, NodeData, str, EdgeData] = UndirectedGraph.Graph()
    leaf_cnter = 0
    processed = set()
    for l in lines[1:]:
        src, dst = tuple(l.split('->'))
...
score = math.inf
new_tree = unrooted_parisomny(g_per_idx)
