...
from FuzzyScoreSpectrums import score_spectrums, top_n_peptides_including_last_place_ties
...
def sequence_cyclic_peptide(
    ...
    while len(leaderboard) > 0:
        ...
        for p in leaderboard:
            ...
            if p_mass + mass_tolerance > cyclopeptide_experimental_mass:
                continue
            elif p_mass - mass_tolerance <= cyclopeptide_experimental_mass <= p_mass + mass_tolerance:
                p_theoretical_spectrum = theoretical_spectrum_of_cyclic_peptide(p, mass_table=mass_table)
                p_score = score_spectrums(p_theoretical_spectrum, cyclopeptide_experimental_spectrum, mass_tolerance=score_tolerance)
                leader_theoretical_spectrum = theoretical_spectrum_of_cyclic_peptide(leader_peptide,
                                                                                     mass_table=mass_table)
                leader_score = score_spectrums(leader_theoretical_spectrum, cyclopeptide_experimental_spectrum, mass_tolerance=score_tolerance)
        ...
        leaderboard = top_n_peptides_including_last_place_ties(
            ...
            n,
            mass_table=mass_table,
            mass_tolerance=score_tolerance)
