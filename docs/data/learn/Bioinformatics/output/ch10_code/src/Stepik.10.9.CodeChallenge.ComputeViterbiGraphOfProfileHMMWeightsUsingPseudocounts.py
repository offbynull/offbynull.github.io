...
with open('/home/user/Downloads/dataset_240403_14(1).txt') as f:
    lines = f.read().splitlines(keepends=False)
emitted_seq = list(lines[0].strip())
threshold = float(lines[2].split()[0])
pseudocount = float(lines[2].split()[1])
symbols = list(lines[4].split())
sequences = [list(l) for l in lines[6:]]
