...
# Load scoring matrix
#
indel_penalty = -2  # specified in problem statement
mismatch_penalties: Dict[Tuple[str, str], int] = {}
for ch1, ch2 in product('ABCDEFGHIJKLMNOPQRSTUVWXYZ', repeat=2):
    mismatch_penalties[(ch1, ch2)] = 1 if ch1 == ch2 else -2


#
# Load sequences
#
with open('/home/user/Downloads/dataset_240306_7.txt', mode='r', encoding='utf-8') as f:
    data = f.read()
...
for row in grid:  # create free-rides from source node to every node in the first column
    ...
    next_edge_id += 1
for src_n in grid[-1]:  # create free-rides from every node in last row to dst node
    if src_n == to_node:
        continue
