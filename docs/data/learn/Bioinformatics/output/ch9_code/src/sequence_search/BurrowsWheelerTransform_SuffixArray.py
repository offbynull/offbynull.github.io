...
def to_bwt(
    ...
    seq_with_counts_rotations_sorted = sorted(
        enumerate(seq_with_counts_rotations),
    ...
    for i, (first_idx, s) in enumerate(seq_with_counts_rotations_sorted):
        ...
        record = BWTRecord(first_ch, first_ch_idx, last_ch, last_ch_idx, first_idx)
        ret.append(record)
...
def main_build():
    ...
    try:
        ...
        print(f' * Last: {[r.last_ch + str(r.last_ch_cnt) + " @ " + str(r.first_idx) for r in bwt_records]}')
    finally:
        print("</div>", end="\n\n")
