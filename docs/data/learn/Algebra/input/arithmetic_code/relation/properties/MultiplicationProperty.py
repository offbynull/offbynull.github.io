...
from relation.Relation import RelationWithDomain, Relation, opposite


def multiplication(rel_with_dom: RelationWithDomain, operand: Node, cache: ExplosionCache) -> set[RelationWithDomain]:
    ...
    if _has_variable(operand):
        ...
        match sign:
            ...
            case '-':
                ...
                options.add(RelationWithDomain(_rel, doms))
            case '0':
                _rel = Relation('=', _lhs, _rhs)  # multiply by 0 means both sides equal to 0
                options.add(RelationWithDomain(_rel, doms))
            case _:
